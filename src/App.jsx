      ...
      <div className="relative w-full max-w-md mx-4">
        ...
        <form onSubmit={handleLogin} className="bg-slate-800/60 backdrop-blur border border-slate-700/50 rounded-2xl p-8 shadow-2xl">
          <div className="space-y-4">
            <div>
              ...
              <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                className="w-full bg-slate-700/50 border border-slate-600 text-white placeholder-slate-500 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent transition"
          ...
          <button type="submit" disabled={loading}
            className="mt-6 w-full bg-gradient-to-r from-sky-500 to-violet-500 hover:from-sky-400 hover:to-violet-400 text-white font-semibold py-3 rounded-xl transition-all duration-200 shadow-lg shadow-sky-500/20 disabled:opacity-50 disabled:cursor-not-allowed">
...
function AgentView({ user, onToast }) {
  ...
  const [aForm, setAForm] = useState({ client_name: "", visit_type: "親訪", notes: "" });
  ...
  const loadActivities = useCallback(async () => {
    ...
    const { data } = await supabase.from("activities").select("*").gte("date", start).lte("date", end).eq("user_id", user.id).order("date");
    setActivities(data || []);
  }, [calYear, calMonth, user.id]);
  ...
  async function handleAddActivity() {
    ...
    setASaving(true);
    const { error } = await supabase.from("activities").insert({ user_id: user.id, date: modalDate, ...aForm });
    setASaving(false);
    if (error) { onToast("新增失敗：" + error.message, "error"); return; }
    onToast("✓ 活動紀錄已新增");
    setAForm({ client_name: "", visit_type: "親訪", notes: "" });
    setModalDate(null);
    loadActivities();
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex gap-2 bg-slate-800/50 rounded-2xl p-1 border border-slate-700/50">
        {[{id:"calendar",label:"📅 個人行事曆"},{id:"sales",label:"💰 業績回報"}].map(t => (
              ...
              {Array.from({length: daysInMonth}).map((_, i) => {
                ...
                return (
                  <button key={day} onClick={() => setModalDate(dateStr)}
                    className={`min-h-[72px] rounded-xl p-1.5 text-left border transition-all hover:border-sky-500/50 hover:bg-sky-500/5 ${isToday ? "border-sky-500/60 bg-sky-500/10" : "border-slate-700/50 bg-slate-800/30"}`}>
                    ...
                    <div className="space-y-0.5">
                      {entries.slice(0,3).map(e => {
                        ...
                        return <div key={e.id} className={`text-[10px] ${c.bg} ${c.text} rounded px-1 py-0.5 truncate font-medium`}>{e.client_name}</div>;
                      })}
        ...
        <div className="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-6">
          ...
          <form onSubmit={handleAddSale} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <Field label="日期 *"><input type="date" value={sForm.date} onChange={e => setSForm(p=>({...p,date:e.target.value}))} className="w-full bg-slate-700/50 border border-slate-600 text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" required /></Field>
              <Field label="客戶姓名 *"><input type="text" value={sForm.client_name} onChange={e => setSForm(p=>({...p,client_name:e.target.value}))} placeholder="王小明" className="w-full bg-slate-700/50 border border-slate-600 text-white placeholder-slate-500 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" /></Field>
            </div>
            <Field label="險種 / 備註"><input type="text" value={sForm.product_name} onChange={e => setSForm(p=>({...p,product_name:e.target.value}))} placeholder="終身壽險、醫療險..." className="w-full bg-slate-700/50 border border-slate-600 text-white placeholder-slate-500 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" /></Field>
            <div className="grid grid-cols-2 gap-4">
              <Field label="受理業績 (元)"><input type="number" value={sForm.accepted_amount} onChange={e => setSForm(p=>({...p,accepted_amount:e.target.value}))} placeholder="0" min="0" className="w-full bg-slate-700/50 border border-slate-600 text-white placeholder-slate-500 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" /></Field>
              <Field label="核實業績 (元)"><input type="number" value={sForm.confirmed_amount} onChange={e => setSForm(p=>({...p,confirmed_amount:e.target.value}))} placeholder="0" min="0" className="w-full bg-slate-700/50 border border-slate-600 text-white placeholder-slate-500 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" /></Field>
            </div>
            <button type="submit" disabled={sSaving}
              className="w-full py-3 rounded-xl bg-gradient-to-r from-emerald-500 to-sky-500 hover:from-emerald-400 hover:to-sky-400 text-white font-semibold text-sm transition-all shadow-lg disabled:opacity-50">
      ...
      {modalDate && (
        <Modal title={`新增拜訪 — ${modalDate}`} onClose={() => setModalDate(null)}>
          <div className="space-y-4">
            {(actByDate[modalDate]||[]).length > 0 && (
              <div className="space-y-2 mb-2">
                <p className="text-xs text-slate-400 font-semibold uppercase tracking-widest">當日紀錄</p>
                {(actByDate[modalDate]||[]).map(e => {
                  const c = VISIT_COLORS[e.visit_type];
                  return <div key={e.id} className={`flex items-center gap-2 ${c.bg} rounded-xl px-3 py-2`}><span className={`w-2 h-2 rounded-full ${c.dot}`} /><span className={`text-sm font-medium ${c.text}`}>{e.visit_type}</span><span className="text-sm text-slate-300">{e.client_name}</span></div>;
                })}
              </div>
            )}
            <div className="border-t border-slate-700 pt-4 space-y-3">
              <p className="text-xs text-slate-400 font-semibold uppercase tracking-widest">新增拜訪</p>
              <Field label="客戶姓名 *"><input type="text" value={aForm.client_name} onChange={e => setAForm(p=>({...p,client_name:e.target.value}))} placeholder="王小明" className="w-full bg-slate-700/50 border border-slate-600 text-white placeholder-slate-500 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" /></Field>
              <Field label="拜訪類型"><select value={aForm.visit_type} onChange={e => setAForm(p=>({...p,visit_type:e.target.value}))} className="w-full bg-slate-700/50 border border-slate-600 text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500">{VISIT_TYPES.map(t => <option key={t}>{t}</option>)}</select></Field>
              <Field label="備註"><input type="text" value={aForm.notes} onChange={e => setAForm(p=>({...p,notes:e.target.value}))} placeholder="討論內容..." className="w-full bg-slate-700/50 border border-slate-600 text-white placeholder-slate-500 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" /></Field>
              <button onClick={handleAddActivity} disabled={aSaving}
                className="w-full py-3 rounded-xl bg-gradient-to-r from-sky-500 to-violet-500 text-white font-semibold text-sm transition-all disabled:opacity-50">
...
function ManagerView({ user, onToast }) {
  ...
  const agentBreakdown = useMemo(() => {
    const map = {};
    [...activities, ...sales].forEach(r => {
      if (!map[r.user_id]) {
        const profile = agents.find(a => a.id === r.user_id);
        map[r.user_id] = { name: profile?.name || "未知", acts: {親訪:0,跨售訪:0,議題訪:0}, accepted:0, confirmed:0 };
      }
    });
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <>
          ...
          {agentFilter === "all" && agentBreakdown.length > 0 && (
            <section>
              ...
              <div className="space-y-3">
                {agentBreakdown.map(([uid, data]) => (
                  <div key={uid} className="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-5">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm bg-slate-700/30 rounded-xl p-3">
          ...
          {activities.length > 0 && (
            <section>
              <div className="flex items-center gap-2 mb-3"><span>📋</span><h2 className="font-bold text-slate-200 text-sm uppercase tracking-widest">拜訪明細清單</h2></div>
              <div className="bg-slate-800/50 border border-slate-700/50 rounded-2xl overflow-hidden">
                <div className="divide-y divide-slate-700/50">
                  {activities.slice().sort((a,b)=>b.date.localeCompare(a.date)).map(act => {
                    const c = VISIT_COLORS[act.visit_type];
                    const agentName = agents.find(a=>a.id===act.user_id)?.name || "—";
                    return (
                      <div key={act.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-slate-700/30 transition">
                        <span className={`w-2 h-2 rounded-full flex-shrink-0 ${c.dot}`} />
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-white truncate">{act.client_name}</p>
                          <p className="text-xs text-slate-500">{agentName} · {act.date}</p>
                        </div>
...
export default function App() {
  ...
  if (!currentUser) return <AuthPage onLogin={u => setCurrentUser(u)} />;
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white">
      ...
      <main className="max-w-3xl mx-auto px-4 py-6 relative">
        {isManager ? <ManagerView user={currentUser} onToast={showToast} /> : <AgentView user={currentUser} onToast={showToast} />}
      </main>
